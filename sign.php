...
$login = 0;
  ...
  if(isset($_SESSION['msg'])){
	...
	if(isset($_POST['btnLogin'])) {
		...
		$myQuery = mysql_query("SELECT roleId FROM users  WHERE username='$username' AND number='$password'")
					     or die(mysql_error()) ;
	...
	if(mysql_num_rows($myQuery)>0){
      ...
      while($row=mysql_fetch_array($myQuery)){
        if($row['roleId']==1){
          ...
          $_SESSION['role'] = "Student/Faculty";
          header("Location: profile");
        }elseif($row['roleId']==2){
          $msg = "Unauthorized Access! Not a student or faculty!";
        }
      }
	}else{
	    $_SESSION['login'] = $login;
  ...
  <body>
    ...
    <div class="">
      <form class="form-signin" role="form" data-toggle="validator" id="signIn" method="post">
		...
		<h4 class="help-block" style="color:red" align="center"><?php if(isset($msg)) { echo $msg;} unset($_SESSION['msg']); ?></h4>
